...
module.exports = {
  ...
  fetchAll: (params, populate) => {
    ...
    const withRelated = populate || Payment.associations
      .filter(ast => ast.autoPopulate !== false)
      .map(ast => ast.alias);

    const filters = convertRestQueryParams(params);

    return Payment.query(buildQuery({ model: Payment, filters }))
  ...
  count: (params) => {
    ...
    return Payment.query(buildQuery({ model: Payment, filters: _.pick(filters, 'where') })).count();
  },
